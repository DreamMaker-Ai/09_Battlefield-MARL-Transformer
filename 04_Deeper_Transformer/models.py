...
class MarlTransformerModel(tf.keras.models.Model):
    ...
    =================================================================
     ...
     multi_head_attention_model_  ((None, 15, 256),        526080
     2 (MultiHeadAttentionModel)   (None, 2, 15, 15))

     multi_head_attention_model_  ((None, 15, 256),        526080
     3 (MultiHeadAttentionModel)   (None, 2, 15, 15))

     q_logits_model (QLogitsMode  (None, 15, 5)            395525
     l)

    =================================================================
    Total params: 2,758,789
    Trainable params: 2,758,789
    ...
    def __init__(self, config, **kwargs):
        ...
        self.mha3 = MultiHeadAttentionModel(config=self.config)

        self.mha4 = MultiHeadAttentionModel(config=self.config)
    ...
    def call(self, x, mask, training=True):
        ...
        """ Multi Head Self-Attention layer 3 """
        # features_mha3: (None,n,hidden_dim),
        # score3: (None,num_heads,n,n)
        features_mha3, score3 = self.mha3(features_mha2, mask, training=training)

        """ Multi Head Self-Attention layer 4 """
        # features_mha24: (None,n,hidden_dim),
        # score4: (None,num_heads,n,n)
        features_mha4, score4 = self.mha4(features_mha3, mask, training=training)

        """ Q network (logits output) """
        q_logits = self.q_net(features_mha4, mask, training=training)  # (None,n,action_dim)

        return q_logits, [score1, score4]
    ...
    def build_graph(self, mask):
        ...
        features_mha3, score3 = self.mha3(features_mha2, mask, training=True)
        features_mha4, score4 = self.mha4(features_mha3, mask, training=True)

        q_logits = self.q_net(features_mha4, mask)

        model = tf.keras.models.Model(
            inputs=[x],
            outputs=[q_logits, [score1, score4]],
