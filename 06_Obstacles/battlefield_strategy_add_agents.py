...
from config_add_agents import ConfigAddAgents
from add_agents_to_env import add_reds
...
class BattleFieldStrategy(gym.Env):
    def __init__(self):
        ...
        self.config_add_agents = ConfigAddAgents()  # Add reds during the episode
        self.add_force = None
    ...
    def reset(self):
        ...
        self.config_add_agents.reset(self.config)
        self.add_force = 0  # Flag for adding red agents once
    ...
    def step(self, actions):
        ...
        """ Add red agents """
        _, total_reds_force, _, _ = compute_current_total_ef_and_force(self.reds)

        if (total_reds_force <= self.config.R0 / 3) and (self.add_force == 0):
            self.add_force = 1
            added_reds = add_reds(self.reds,
                                  self.config,
                                  self.battlefield,
                                  self.blues,
                                  self.config_add_agents)

            self.reds = self.reds + added_reds

            for added_red in added_reds:
                dones[added_red.id] = False
                rewards[added_red.id] = 0.0
