...
from battlefield_strategy_add_agents import BattleFieldStrategy
...
class Tester:
        ...
        for _ in range(self.env.config.max_episodes_test_play):
            ...
            while not dones['all_dones']:
                ...
                for idx in next_alive_agents_ids:
                    ...
                    # process added agents
                    if len(next_alive_agents_ids) > len(self.alive_agents_ids):
                        dif_ids = set(next_alive_agents_ids) ^ set(self.alive_agents_ids)

                        red_id = 'red_' + str(idx)
                        self.frames[red_id] = \
                            deque([next_obserations[red_id]] * self.n_frames, maxlen=self.n_frames)
                        #self.states[red_id] = \
                        #    np.concatenate(self.frames[red_id], axis=2).astype(np.float32)
    ...
    def save_test_conds(self):
        test_conds = {
            ...
            'add num red platoons range': self.env.config_add_agents.red_platoons_add,
            'add num red companies range': self.env.config_add_agents.red_companies_add,
            'add efficiency red range': self.env.config_add_agents.efficiencies_red_add,
        }
