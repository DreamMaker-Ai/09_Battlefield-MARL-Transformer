...
def main():
    ...
    for f, c in zip(filelist, colorlist):
        ...
        plt.plot(correct_time / 3600, reds_force, linestyle='solid', color=c, alpha=0.7,
                 linewidth=1, label=f)

    # plt.yscale('log')
    plt.title('Mean remaining effective force of reds vs Learning time')
    plt.grid(which="both")
    plt.minorticks_on()
    plt.legend()
    ...
    plt.savefig(str(savedir) + '/' + savename + '-time.png')
